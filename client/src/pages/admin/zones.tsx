...
export default function AdminZones() {
  ...
  const addZoneMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Erro",
        description: error.message || "Erro ao adicionar zona. Tente novamente.",
        variant: "destructive",
      });
    },
  });
  ...
  const editZoneMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Erro",
        description: error.message || "Erro ao atualizar zona. Tente novamente.",
        variant: "destructive",
      });
    },
  });
  ...
  const toggleZoneActiveMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Erro",
        description: error.message || "Erro ao alterar status da zona. Tente novamente.",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Gerencie as zonas de estacionamento do sistema rotativo municipal." />
      </Helmet>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ocupação atual</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {zones?.length > 0 ? (
                  zones.map((zone: any) => (
                    <tr key={zone.id} className={!zone.active ? "bg-gray-50" : ""}>
                      ...
                      <td className="px-4 py-4">
                        <div className="text-sm text-gray-900 max-w-md">{zone.description || "Sem descrição"}</div>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="w-full bg-gray-200 rounded-full h-2.5 mb-1">
                          <div 
                            className={`h-2.5 rounded-full ${
                              ...
                              zone.occupancyRate > 60 ? 'bg-yellow-500' : 
                              'bg-green-500'
                            }`} 
                            style={{ width: `${zone.occupancyRate || 0}%` }}
                          ></div>
                        </div>
                        <div className="text-xs text-gray-500 text-right">{zone.occupancyRate || 0}% ocupada</div>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap">
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          ...
                          <Button 
                            ...
                            className={zone.active ? "text-amber-600 hover:text-amber-700" : "text-green-600 hover:text-green-700"}
                          >
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir a zona "{selectedZone?.name}"? Esta ação não pode ser desfeita e pode afetar as permissões de estacionamento existentes.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
