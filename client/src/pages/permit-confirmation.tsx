  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // const response = await apiRequest("GET", `/api/permits/${permitId}`);
        ...
        if (storedData) {
          ...
          setConfirmationData({
            id: permitId || "1",
            transactionCode: "EF" + new Date().toISOString().substring(0, 10).replace(/-/g, "") + Math.floor(Math.random() * 1000),
            vehicle: {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Confirmação de compra de permissão de estacionamento." />
      </Helmet>
      ...
      <Card className="max-w-md mx-auto">
        <CardContent className="p-6">
          <div className="text-center mb-6">
            ...
            <h2 className="text-xl font-bold text-gray-800">Permissão Adquirida!</h2>
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6">
            <div className="flex justify-between mb-2">
              ...
              <span className="font-semibold">{confirmationData.transactionCode}</span>
            </div>
            <div className="flex justify-between mb-2">
              <span className="text-gray-600">Placa:</span>
              <span className="font-semibold">{confirmationData.vehicle.licensePlate}</span>
            </div>
            <div className="flex justify-between mb-2">
              <span className="text-gray-600">Validade:</span>
              <span className="font-semibold">
                {formatDateTime(confirmationData.startTime)} até {formatDateTime(confirmationData.endTime)}
              </span>
            </div>
            <div className="flex justify-between mb-2">
              <span className="text-gray-600">Zona:</span>
              <span className="font-semibold">{confirmationData.zone.name}</span>
            </div>
            <div className="flex justify-between text-lg border-t border-gray-300 pt-2 mt-2">
              ...
              <span className="text-secondary font-semibold">{formatMoney(confirmationData.amount)}</span>
            </div>
          </div>
          
          <div className="text-center mb-6">
            <p className="text-sm text-gray-600 mb-2">
              Guarde o código de consulta para verificar o status da sua permissão posteriormente.
            </p>
            <Button
              variant="outline"
          ...
          <div className="flex justify-center">
            <Link href="/">
              <Button className="bg-primary hover:bg-primary-light text-white">Concluir</Button>
            </Link>
          </div>
        </CardContent>
      </Card>
