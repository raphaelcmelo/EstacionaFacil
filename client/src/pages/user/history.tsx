  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Visualize o histórico completo de suas permissões de estacionamento." />
      </Helmet>
                  ...
                  <tr key={history.id}>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className={`px-2 py-1 text-xs rounded-full inline-flex items-center
                        ${history.paymentStatus === 'COMPLETED' ? 'bg-green-100 text-green-800' : 
                          history.paymentStatus === 'FAILED' ? 'bg-red-100 text-red-800' :
                          'bg-yellow-100 text-yellow-800'}`}>
                        {history.paymentStatus === 'COMPLETED' ? 'Pago' :
                          history.paymentStatus === 'FAILED' ? 'Falhou' :
                          history.paymentStatus === 'PENDING' ? 'Pendente' :
        ...
        {permitHistory?.length > 0 && (
          <div className="p-4 border-t border-gray-200 flex justify-between items-center">
            <div className="text-sm text-gray-600">
              Mostrando {offset + 1}-{Math.min(offset + permitHistory.length, offset + limit)} resultados
            </div>
