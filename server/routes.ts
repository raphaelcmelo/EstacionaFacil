...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/register", async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof ZodError) {
        return res.status(400).json({ message: "Dados inválidos", errors: formatErrors(error) });
      }
  ...
  app.post("/api/vehicles", async (req: any, res: Response) => {
    ...
    try {
      ...
      const existingVehicle = await storage.getVehicleByLicensePlate(data.licensePlate);
      if (existingVehicle) {
  ...
  app.get("/api/zones", async (req: Request, res: Response) => {
    const zones = await storage.listActiveZones();
  ...
  app.get("/api/prices/:zoneId", async (req: Request, res: Response) => {
    ...
    if (!priceConfig) {
      return res.status(404).json({ message: "Configuração de preço não encontrada" });
    }
  ...
  app.get("/api/permits/active", async (req: any, res: Response) => {
    ...
    const activePermits = await storage.listActiveParkingPermitsByUserId(req.user.id);
  ...
  app.get("/api/permits/history", async (req: any, res: Response) => {
    ...
    const permitHistory = await storage.listParkingPermitHistory(req.user.id, limit, offset);
  ...
  app.post("/api/permits/check", async (req: Request, res: Response) => {
    try {
      ...
      const activePermit = await storage.getActiveParkingPermitByLicensePlate(data.licensePlate);
  ...
  app.post("/api/fiscal/verify", async (req: any, res: Response) => {
    if (!req.user || (req.user.role !== "FISCAL" && req.user.role !== "ADMIN")) {
      return res.status(403).json({ message: "Acesso negado" });
    }
    ...
    try {
      ...
      const activePermit = await storage.getActiveParkingPermitByLicensePlate(licensePlate);
  ...
  app.post("/api/fiscal/infringement", async (req: any, res: Response) => {
    ...
    try {
      const { licensePlate, infringementType, notes, evidencePhotos, latitude, longitude } = req.body;
      
      if (!licensePlate || !infringementType) {
        return res.status(400).json({ message: "Placa e tipo de infração são obrigatórios" });
      }
  ...
  app.get("/api/fiscal/activity", async (req: any, res: Response) => {
    ...
    try {
      const limit = parseInt(req.query.limit as string) || 10;
      const fiscalActions = await storage.listFiscalActionsByFiscalId(req.user.id, limit);
