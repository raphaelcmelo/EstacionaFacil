...
export const users = pgTable("users", {
  ...
  role: text("role", { enum: Object.values(UserRole) }).default("CITIZEN").notNull(),
...
export const parkingPermits = pgTable("parking_permits", {
  ...
  zoneId: integer("zone_id").references(() => zones.id).notNull(),
  priceId: integer("price_id").references(() => priceConfigs.id).notNull(),
  amount: decimal("amount", { precision: 10, scale: 2 }).notNull(),
  paymentStatus: text("payment_status", { enum: Object.values(PaymentStatus) }).default("PENDING").notNull(),
...
export const fiscalActions = pgTable("fiscal_actions", {
  id: serial("id").primaryKey(),
  fiscalId: integer("fiscal_id").references(() => users.id).notNull(),
...
export const verifications = pgTable("verifications", {
  id: serial("id").primaryKey(),
  fiscalActionId: integer("fiscal_action_id").references(() => fiscalActions.id).notNull(),
  permitId: integer("permit_id").references(() => parkingPermits.id).notNull(),
...
export const infringements = pgTable("infringements", {
  ...
  fiscalActionId: integer("fiscal_action_id").references(() => fiscalActions.id).notNull(),
  vehicleId: integer("vehicle_id").references(() => vehicles.id).notNull(),
  infringementType: text("infringement_type", { enum: Object.values(InfringementType) }).notNull(),
  evidencePhotos: json("evidence_photos").$type<string[]>(),
  notes: text("notes"),
  status: text("status", { enum: Object.values(InfringementStatus) }).default("REGISTERED").notNull()
});
...
export const insertPriceConfigSchema = createInsertSchema(priceConfigs).omit({ id: true, createdAt: true, updatedAt: true });
export const insertParkingPermitSchema = createInsertSchema(parkingPermits).omit({ id: true, createdAt: true, updatedAt: true });
export const insertFiscalActionSchema = createInsertSchema(fiscalActions).omit({ id: true, actionTime: true });
export const insertVerificationSchema = createInsertSchema(verifications).omit({ id: true });
export const insertInfringementSchema = createInsertSchema(infringements).omit({ id: true });
